...
for (var task of arr) {
    ...
    <div class="row bg-dark">
      ...
      <div class="col-2  py-3 bg-success d-flex justify-content-center pointer" onclick="beCompleted('${task.id}')"><i class="fa-solid fa-check fs-3  d-flex align-items-center"></i></div>
      <div class="col-2  py-3 bg-danger d-flex justify-content-center pointer" onclick="deltedTodo('${task.id}')"><i class="fa-solid fa-trash fs-3  d-flex align-items-center"></i></div>
    </div>
  </div>
    `
}
...
function beCompleted(id){
 var foundedIndex = allTodos.findIndex((task)=> task.id == id );
...
function DisplayAccordingToSelectValue (){
    switch(mySelect.options[mySelect.options.selectedIndex].value){
        case 'all':
            ...
            break;
        case 'completed':
           var compltedFilterd =  allTodos.filter(function(list){ return list.isCompleted == true});
